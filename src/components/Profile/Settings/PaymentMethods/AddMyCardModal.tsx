...
const dispatcher = (dispatch) => ({
  ...
  addCreditCard: (params) => dispatch(addMyCCAction(params)),
...
const AddMyCardModal = ({
  ...
  const [makeAsDefault, setMakeAsDefault] = useState<boolean>(false)
  const [billingAddress, setBillingAddress] = useState({} as ShippingAddress)
  const [paymentError, setPaymentError] = useState('')
  useEffect(() => {
    getAddressDetails(atgUserId)
  }, [])
  useEffect(() => {
  ...
  const addNewPaymentCard = async () => {
    ...
    } else if (addResult?.data?.response?.message) {
      setPaymentError(addResult?.data?.response?.message)
    }
  }
  ...
  return (
    ...
    >
      <ScrollContainer>
        ...
        <RadioButton
          disabled={false}
          selected={makeAsDefault}
          checkboxStyle={true}
          onPress={() => setMakeAsDefault(!makeAsDefault)}
          formId={FORM_ID}
...
export default connector(AddMyCardModal)
