...
const dispatcher = dispatch => ({
  clearFormErrorMessages: () => dispatch(clearFormErrorMessagesAction({ formId: CAFE_ERROR_MODAL })),
})
...
const CafeErrorModal = ({ isOpen, onDismiss, formErrors, clearFormErrorMessages }: Props) => {
  let errorMessage = ''
  if (formErrors[CAFE_ERROR_MODAL] && formErrors[CAFE_ERROR_MODAL].body) {
    errorMessage = formErrors[CAFE_ERROR_MODAL].body
  }
  ...
  return (
    <Alert
      isOpen={ isOpen }
      onDismiss={ onDismiss }
      title="Something went wrong with Cafe"
      description={ errorMessage }
      cancelText="Dismiss"
    />
  )
}
export default connector(CafeErrorModal)
