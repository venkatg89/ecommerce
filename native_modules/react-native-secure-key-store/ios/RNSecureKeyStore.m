...
static NSString *accessGroupRegular = @"P257MFRRYL.com.barnesandnoble.app";
static NSString *accessGroupEnterpise = @"32RT7R8955.com.barnesandnoble.app";
static NSString *nookService = @"com.bn.KeyChain.Service.AppInfo";
static NSString *nookKey = @"com.barnesandnoble.B-N-eReader";
...
// We don't require Main Queue-set up.
...
- (NSMutableDictionary *)searchDictionaryForNook:(NSString *)serviceName {
    NSMutableDictionary *searchDictionary = [[NSMutableDictionary alloc] init];
    
    [searchDictionary setObject:(id)kSecClassGenericPassword forKey:(id)kSecClass];
    [searchDictionary setObject:[serviceName dataUsingEncoding:NSUTF8StringEncoding] forKey:(id)kSecAttrAccount];
    [searchDictionary setObject:serviceName forKey:(id)kSecAttrService];
#if TARGET_IPHONE_SIMULATOR
#else
    [searchDictionary setObject:@"P257MFRRYL.com.barnesandnoble" forKey:(id)kSecAttrAccessGroup];
#endif
...
#if ENTERPRISE
    NSString* accessGroup = accessGroupRegular;
#else
    NSString* accessGroup = accessGroupEnterpise;
#endif
...
- (NSDictionary *)searchKeychainCopyMatchingNook:(NSString *)service {
    NSMutableDictionary *searchDictionary = [self searchDictionaryForNook:service];
    [searchDictionary setObject:(id)kSecMatchLimitOne forKey:(id)kSecMatchLimit];
    [searchDictionary setObject:(id)kCFBooleanTrue forKey:(id)kSecReturnData];
    [searchDictionary setObject:(id)kCFBooleanTrue forKey:(id)kSecReturnAttributes];
    CFTypeRef cfValue = NULL;
    OSStatus status = SecItemCopyMatching((CFDictionaryRef)searchDictionary, &cfValue);
    if (status == noErr) {
        NSLog(@"no Error");
        // There is an existing keychain item => attributes
        NSDictionary *result = CFBridgingRelease(cfValue);
        return result;
    }
    if (cfValue) {
        CFRelease(cfValue);
...
RCT_EXPORT_METHOD(getNook:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject)
{
    @try {
        NSDictionary *appInfo = nil;
        NSDictionary *result = [self searchKeychainCopyMatchingNook:nookService];
        
        if ( result != nil ) {
            NSData *resultData = [result objectForKey:(id)kSecValueData];
            NSKeyedUnarchiver *unarchiver = [[NSKeyedUnarchiver alloc] initForReadingWithData:resultData];
            NSDictionary *myDictionary = [unarchiver
                                          decodeObjectForKey:nookKey];
            [unarchiver finishDecoding];
            if ( ![myDictionary isEqual:[NSNull null]]) {
                appInfo = myDictionary;
            }
        }
        resolve(appInfo);
    }
